...
@LLMRegistry.register("VILA_M3")
class VILA_M3:
    def __new__(cls, model_path: str, args: Any) -> Any:
        from models.VILA_M3.VILA_M3 import VILA_M3
        return VILA_M3(model_path, args)
...
class Qwen3_VL:
    def __new__(cls, model_path: str, args: Any) -> Any:
        # 遵循 Qwen 系列的各种版本判断逻辑
        if os.environ.get("use_vllm", "True") == "True":
            # 假设您未来会添加 vLLM 实现，路径规范仿照 Qwen2
            from models.Qwen3_VL.Qwen3_VL_vllm import Qwen3_VL
        else:
            # 使用刚才编写的 HuggingFace 版本
